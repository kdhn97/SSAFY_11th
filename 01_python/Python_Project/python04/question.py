...
tuple
- 순서가 있는 시퀀스 타입
- 순회가 가능한 iterable 데이터
- 여러 개의 데이터를 담을 수 있는 colletion
...
# 애초에 코드를 그렇게 안 쓰면 되는거아닌가?
    # 매개변수로 넘겨 받은 데이터가 변경되지 않도록 코드를 쓴다
    # 지극히 맞는 말, 그렇게 하라고 일부로 tuple로 한번더
def some_func(*args):
    print(args) # (1, 2, 3, 4)
...
def 함수_이름(매개변수, 매개변수): (이름이 있어서 나중에 다시 부를 수 있음)
    함수 할 일
- 코드 재사용 가능
- a데이터도, b데이터도, c데이터도 매번 함수를 거쳐야 한다.
